                                      ...
                                      double* smallJac)
{
  ...
  {
    for (unsigned int bigCol = 0; bigCol < fullJac->sizeCols; bigCol++)
    {
                                                 ...
                                                 GB_INTERNAL_NLS_DATA *nls)
{
  ...
  if ((sdirk_first_stage || esdirk_first_stage) && gbData->type == GM_TYPE_DIRK)
  {
    ...
    if (jac_called || stepSize != lastStepSize)
    {
      ...
      jacobian_SR_DIRK_assemble(data, threadData, gbData, nls, ode_pattern, nls->jacobian_callback, nls->real_nls_jacs[0]);
