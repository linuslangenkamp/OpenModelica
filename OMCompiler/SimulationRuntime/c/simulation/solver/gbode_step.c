...
int expl_diag_impl_RK_MR(DATA* data, threadData_t* threadData, SOLVER_INFO* solverInfo)
{
  ...
  for (stage = 0; stage < nStages; stage++) {
    ...
    if (gbfData->tableau->A[stage * nStages + stage_] == 0) {
      ...
      if (gbfData->tableau->svp != NULL && gbfData->tableau->svp->type[stage_] == SVP_LINEAR_COMBINATION)
      {
        /* linear combination stage-value-predictors (highest priority) */
        gbInternalLinearCombinationSVP(gbfData->tableau->svp, stage_, nFastStates, gbfData->stepSize, gbfData->kCurrPacked, gbfData->yOldPacked, nlsData->nlsxOld);
      }
      else if (dense_output_valid && gbfData->tableau->svp != NULL && gbfData->tableau->svp->type[stage_] == SVP_DENSE_OUTPUT)
      {
        /* dense output stage-value-predictor */
        double theta = (gbfData->time + gbfData->tableau->c[stage_] * gbfData->stepSize - gbfData->extrapolationBaseTime) / gbfData->extrapolationStepSize;
